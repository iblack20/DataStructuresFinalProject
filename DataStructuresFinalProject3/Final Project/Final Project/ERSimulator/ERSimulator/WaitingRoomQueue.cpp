...
void WaitingRoomQueue::update(int t, WaitingRoomQueue& h)
{ 
	...
	if (WholePatients >= 1.00)										// will loop through number of pateints arriving per minute (Arrival rate/60)
	{
		...
		Resident R1 = *(Residents[randnum]);
		R1.setPatientTime(t);
		if (R1.getPrioritynum() > 0 && R1.getPrioritynum() <= 10) {
			lowPriorityQueue.push(R1);
		}
		else {
			highPriorityQueue.push(R1);
		}
		Residents.erase(Residents.begin() + randnum);
		// Pushes Resident from vector into priority queue
		WholePatients--;
	}
}
